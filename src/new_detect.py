    ...
    while True:
        ...
        if results.face_landmarks:
            ...
            mesh_points_3D = np.array(
                [[n.x, n.y, n.z] for n in results.face_landmarks[0]]
            )
            ...
            # Eye mouse signal (use left eye only for stability)
            # -----------------------------
            dx, dy = l_dx, l_dy
            ...
            if key == ord('c'):
                print(f"Calibrations : {len(calib_data)}")
                calib_data.append((dx, dy))

                if len(calib_data) >= 20:
                    calib_data = np.array(calib_data)
                    min_dx, max_dx = calib_data[:,0].min(), calib_data[:,0].max()
                    min_dy, max_dy = calib_data[:,1].min(), calib_data[:,1].max()
                    calibrated = True
            ...
            if calibrated:
                ...
                # normalize safely
                nx = normalize(dx, min_dx, max_dx)
                ny = normalize(dy, min_dy, max_dy)
                ...
                # map to screen
                target_x = float(nx * screen_w)
                target_y = float(ny * screen_h)
